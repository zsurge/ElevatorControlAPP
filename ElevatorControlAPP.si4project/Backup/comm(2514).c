...
SYSERRORCODE_E GetDevInfo ( uint8_t* msgBuf )
{
    ...
    //因文档风格不统一，所以要先取identification
    identification = GetJsonItem(msgBuf,"data",0);
    
    result = modifyJsonItem(msgBuf,"identification",identification,1,buf);
    result = modifyJsonItem(buf,"version",gDevinfo.SoftwareVersion,1,buf);
    result = modifyJsonItem(buf,"appName ",gDevinfo.Model,1,buf);
    result = modifyJsonItem(buf,"regRersion ","8",1,buf);
    result = modifyJsonItem(buf,"regface ","7",1,buf);
    result = modifyJsonItem(buf,"ip ","192.168.31.10",1,buf);
